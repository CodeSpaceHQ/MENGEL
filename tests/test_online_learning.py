...
class TestOnlineLearning(TestCase):
    ...
    def test_sgd_regressor(self):
        result = scikit_online_regressors.run_sgd_regressor(setup.get_datasets_path(), "winequality-red.csv", ';')
        self.assertGreater(result, 0, msg="Failed to beat baseline")
    
    def test_passive_aggressive_regressor(self):
        result = scikit_online_regressors.run_passive_aggressive_regressor(setup.get_datasets_path(), "winequality-red.csv", ';')
        self.assertGreater(result, 0, msg="Failed to beat baseline")
